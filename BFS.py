...
class Algorithm:
    ...
    best_child_node_list = []  # a list for the next nodes to take
    ...
    def find_next(self):
        ...
        if finished is False:
                self.current_node = self.best_child_node_list.pop(0)  # first come first serve
                for x in self.board.list:  # detect current node and its children
                    if x.i == self.current_node.i + 1 and x.j == self.current_node.j \
                            ...
                            or x.i == self.current_node.i and x.j == self.current_node.j - 1:
                        if x.parent is None:  # if the node doesn't have a parent its get appended to the list
                        ...
                        if self.current_node.value == 0:  # if we have node that has value 0 then we have finished
                            finished = True
